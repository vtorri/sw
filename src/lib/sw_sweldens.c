...
struct sw_sweldens_s
{
    sw_scale_fct_base_t const *sfb;
    rational_t                *weights;
    rational_t                *absciss;
    int32_t                    order;
    int32_t                    scale;
...
static int32_t
_lookup0(sw_sweldens_t const *s, int32_t i, int32_t j)
{
    /* int32_t r; */
...
static int32_t
_lookup1(sw_sweldens_t const *s, int32_t k, int32_t i, int32_t j)
{
    int32_t N1;
    int32_t r;

    N1 = sw_scale_fct_base_N1_get(s->sfb);
    r = s->order;
...
static rational_t *
_coefs_get(sw_sweldens_t const *s)
{
    rational_t *coefs;
    int32_t     N1;
    int32_t     N2;
    ...
    N1 = sw_scale_fct_base_N1_get(s->sfb);
    N2 = sw_scale_fct_base_N2_get(s->sfb);
    r = s->order;
...
static rational_t *
_weights_get(sw_sweldens_t const *s)
{
    rational_t       *coefs;
    rational_t       *weights;
    ...
    N1 = sw_scale_fct_base_N1_get(s->sfb);
    N2 = sw_scale_fct_base_N2_get(s->sfb);
    filter = sw_scale_fct_base_filter_rat_get(s->sfb);
    r = s->order;
...
static rational_t *
_moments_get(sw_sweldens_t const *s)
{
    rational_t *weights;
    rational_t *moments;
    int32_t     i;
...
static rational_t *
_coefs_pol_get(sw_sweldens_t const *s)
{
    rational_t *coefs;
    int32_t     N1;
    int32_t     N2;
...
static rational_t
_sw_sweldens_chebyshev_value_get(int32_t n, rational_t const *x)
{
    rational_t un;
    rational_t deux;
    rational_t tmp1;
                         ...
                         rational_t const *tau)
{
    ...
    for (k = 0; k < r; k++)
    {
        ...
        for (i = 0; i < r; i++)
        {
            matrix[k * r + i] = _sw_sweldens_chebyshev_value_get(i, &dk);
        }
    }
                ...
                rational_t const          *tau)
{
    ...
    s = (sw_sweldens_t *)malloc(sizeof(sw_sweldens_t));
    if (!s)
        return NULL;
    ...
    _sw_sweldens_weights_get(s, tau);
...
void
sw_sweldens_delete(sw_sweldens_t *s)
{
    if (!s)
        return;
...
int32_t
sw_sweldens_order_get(sw_sweldens_t const *s)
{
    return s->order;
}

rational_t *
sw_sweldens_absciss_get(sw_sweldens_t const *s)
{
    if (!s)
        return NULL;
...
rational_t *
sw_sweldens_weights_get(sw_sweldens_t const *s)
{
    if (!s)
        return NULL;
